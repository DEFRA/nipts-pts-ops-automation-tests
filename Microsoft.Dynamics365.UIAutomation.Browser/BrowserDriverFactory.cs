    ...
    public static IWebDriver CreateWebDriver(BrowserOptions options)
    {
        ...
        switch (options.BrowserType)
        {
            case BrowserType.Chrome:
                ...
                //{
                //    { "behavior", "allow" },
                //    { "downloadPath", Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "Downloads") }
                //};
                //(driver as ChromeDriver).ExecuteCustomDriverCommand("Page.setDownloadBehavior", settingForHeadlessDownloads);
                break;
            case BrowserType.IE:
